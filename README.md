...
`approxPvWatts = batteryVoltage x chargeCurrent` as a placeholder. A future
...
`live_sample` stores battery V, charge/discharge A, PV W, load W, temp  degC,
...
```
SoC% = clamp((vBat - cutoffV) / (chargeV - cutoffV) x 100, 0, 100)
```
