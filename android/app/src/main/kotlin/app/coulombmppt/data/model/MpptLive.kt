...
// `read(0x0001, 16)` response. Field names follow BLE_PROTOCOL.md s.7.1
...
data class MpptLive(
     ...
     *  bundle), formula `(1000 x hi + lo) / 10`. The vendor app labels this
     ...
     * the registers the app polls (BLE_PROTOCOL.md s.7.1), so we approximate
     * by V_battery x I_charge. That's actually the *battery-input* power
